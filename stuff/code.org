...
** loop over files
#+begin_src python :session :results value :tangle ./../features/extract_features.py
def call_for_files(directory, func, selector='.wav'):
...
#+RESULTS:
: None
** retrieve the cosine similarity betwen a master file and a selected file
This function takes a vector from an analyzed file and a directory of json files to compare it with.
#+begin_src python :session :results value :tangle ./../features/read_features.py
