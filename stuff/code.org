...
* Segmentation
File segmentation in meaningful segments for files longer than 4s. Each segment should be no longer than 4s, and not shorter than 1s (roughly)

#+begin_src python :session :results value :tangle ./../features/segment_audio_old.py
...
# audio = 'dataton2.wav'
  ...
  segment_dir = os.path.join(audio_name + '_segments')
  ...
  if not segment_exists:
    ...
    for i, row in regions.iterrows():
      ...
      segment = s[start:end]
      segments.append = os.path.join(Path(audio).stem + '_segment_{}.wav'.format(i))
...
#+begin_src python :session :results value :tangle ./../features/soundfile.py
  ...
  def analyze(self):
    ...
    if self.segmented:
      for i in range(len(self.segment_names)):
        ...
        json_file = {os.path.join(self.path, self.name): json_file, "start_time": self.start[i], "end_time": self.end[i]}
        with open(os.path.join(path, jsonf), 'w', encoding='utf-8') as f:
          f.write(json.dumps(json_file))
    else:
      ...
      # Add filename header
      json_file = { str(k): v for k,v in json_file.items() }
      json_file = {audiof: json_file, "start_time": self.start[0], "end_time": self.end[0]}
      # Write to file
      with open(os.path.join(path, jsonf), 'w', encoding='utf-8') as f:
        f.write(json.dumps(json_file))
